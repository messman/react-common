...
import { PromiseOutput } from './promise/promise';

const dataRefreshTimeout = 500;
const dataRefreshVisibilityBuffer = 300;
	...
	React.useEffect(() => {
		...
		function onVisibilityChange() {
			if (document.hidden) {
				...
				const timeRemaining = Math.max(dataRefreshTimeout - timeElapsed, dataRefreshVisibilityBuffer);
				if (timeRemaining <= 0) {
					run!(false);
				}
				else {
