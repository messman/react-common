 ...
 */
export function get<T>(key: string, migrations: LocalStorageMigration<T>[] | null, version: string): T | undefined {
	try {
		...
		if (stringItem) {
			...
			if (migratedItem !== item) {
				set(key, migratedItem!.i, version);
			}
...
*/
export function useLocalStorage<T>(key: string, initialValue: T | (() => T), isValid: (value: T) => boolean, migrations: LocalStorageMigration<T>[] | null, version: string): UseLocalStorageReturn<T> {
	const [storedValue, setStoredValue] = React.useState(() => {
		let value: T | undefined = get<T>(key, migrations, version);
